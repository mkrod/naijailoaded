...
const Music: FC<Props> = ({ data, categories }): ReactNode => {
    ...
    const categoryName = activeCategory ? activeCategory.name : "Latest Music";

    const seoTitle = `${categoryName} - Download & Listen ${Number(page) > 1 ? `(Page ${page})` : ""} | ${siteName}`;
        ...
        "@graph": [
            {
                ...
                "itemListElement": [
                    ...
                    { "@type": "ListItem", "position": 2, "name": "Music", "item": `${clientURL}/music` },
                    activeCategory && { "@type": "ListItem", "position": 3, "name": categoryName }
                ].filter(Boolean)
            },
            {
                "@type": "ItemList",
                "name": `${categoryName} Tracks`,
    ...
    return (
        <>
            ...
            <main className={styles[`${mobileClass}container`]}>
                ...
                <nav aria-label="Music pagination" className={styles.pagination_container}>
                    <Pagination
                        renderItem={(item) => (
                            <PaginationItem
                                component={Link}
